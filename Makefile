...
ifeq ($(PLATFORM),PLATFORM_WEB)
    ...
    export PATH         = \
      $(EMSDK_PATH);\
      $(EMSDK_PATH)\clang\$(CLANG_VERSION);\
      $(EMSDK_PATH)\node\$(NODE_VERSION)\bin;\
      $(EMSDK_PATH)\python\$(PYTHON_VERSION);\
      $(EMSDK_PATH)\emscripten\$(EMSCRIPTEN_VERSION);\
      C:\raylib\MinGW\bin:$$PATH
    EMSCRIPTEN          = $(EMSDK_PATH)\emscripten\$(EMSCRIPTEN_VERSION)
endif
...
# Define default C compiler: gcc (or clang++ on macOS/BSD).
CC = g++
...
# Define default make program: Mingw32-make on Windows, make on *nix.
MAKE = mingw32-make
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ifeq ($(PLATFORM_OS),LINUX)
        MAKE = make
...
# Include paths for required headers
INCLUDE_PATHS = -I. -I$(RAYLIB_PATH)/src -I$(RAYLIB_PATH)/src/external
ifneq ($(wildcard /opt/homebrew/include/.*),)
    INCLUDE_PATHS += -I/opt/homebrew/include
endif
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ...
    ifeq ($(PLATFORM_OS),LINUX)
        INCLUDE_PATHS = \
          ...
          -isystem$(RAYLIB_PATH)/release/include \
          -isystem$(RAYLIB_PATH)/src/external
    endif
endif

# Library paths for required libs
LDFLAGS = -L.
ifneq ($(wildcard $(RAYLIB_RELEASE_PATH)/.*),)
    LDFLAGS += -L$(RAYLIB_RELEASE_PATH)
endif
...
# Now Make knows that “all” simply depends on “$(PROJECT_NAME)”, so no recursive calls!
#─────────────────────────────────────────────────────────────────────
#  ↑↑↑  ↑↑↑  ↑↑↑  END OF CHANGE  ↑↑↑  ↑↑↑  ↑↑↑
#─────────────────────────────────────────────────────────────────────
...
$(PROJECT_NAME): $(OBJS)
	$(CC) -o $(PROJECT_NAME)$(EXT) $(OBJS) \
	     $(CFLAGS) $(INCLUDE_PATHS) $(LDFLAGS) $(LDLIBS) -D$(PLATFORM)
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
	...
	find -type f -executable \
	  | xargs file -i \
	  | grep -E 'x-object|x-archive|x-sharedlib|x-executable' \
	  | rev | cut -d ':' -f 2- \
	  | rev | xargs rm -fv
    endif
    ifeq ($(PLATFORM_OS),OSX)
